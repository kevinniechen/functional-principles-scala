...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(x => x).toList map {case (c, o) => (c, o.length)} sorted

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = wordOccurrences(s.foldRight("")((a: Word, b: Word) => a ++ b))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    (occurrences foldRight List[Occurrences](Nil)) {
      case ((ch, oc), r) => {
        (
          for {
            prev_combination <- r; n <- 1 to oc
          } yield (ch, n) :: prev_combination
        ) ++ r
      }
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val ym = y.toMap
    x.foldRight(List[(Char, Int)]()) ((a, b) => (a._1, a._2 - ym.getOrElse(a._1, 0)) :: b).filter(_._2 != 0)
  }
    ...
    def sentenceAnagramsHelper(occurrences: Occurrences): List[Sentence] = occurrences match {
      ...
      case a =>
        for {
          ...
          rest_sentence <- sentenceAnagramsHelper(subtract(occurrences, wordOccurrences(word)))
          if !combination.isEmpty
        } yield word :: rest_sentence
    }
